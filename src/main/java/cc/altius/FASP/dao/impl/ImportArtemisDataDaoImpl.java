...
//                + "o.`FLAG`=1;";
        sql = "UPDATE tmp_erp_order t "
                ...
                + "o.`RO_PRIME_LINE_NO`=t.`RO_PRIME_LINE_NO`, "
                + "o.`ORDER_TYPE`=t.`ORDER_TYPE_IND`, "
                + "o.`CREATED_DATE`=IFNULL(CONCAT(LEFT(t.`ORDER_ENTRY_DATE`,10),' ',MID(t.`ORDER_ENTRY_DATE`,12,8)),NULL), "
                + "o.`PARENT_RO`=t.`PARENT_RO`, "
                + "o.`PARENT_CREATED_DATE`=IFNULL(CONCAT(LEFT(t.`PARENT_ORDER_ENTRY_DATE`,10),' ',MID(t.`PARENT_ORDER_ENTRY_DATE`,12,8)),NULL), "
                + "o.`PLANNING_UNIT_SKU_CODE`=LEFT(t.`ITEM_ID`,12), "
                + "o.`PROCUREMENT_UNIT_SKU_CODE`=IF(LENGTH(t.`ITEM_ID`)=15,t.`ITEM_ID`,NULL), "
                + "o.`QTY`=t.`ORDERED_QTY`, "
                + " o.`ORDERD_DATE`=IFNULL(CONCAT(LEFT(t.`PO_RELEASED_FOR_FULFILLMENT_DATE`,10),' ',MID(t.`PO_RELEASED_FOR_FULFILLMENT_DATE`,12,8)),NULL), "
                + " o.`CURRENT_ESTIMATED_DELIVERY_DATE`=IFNULL(LEFT(t.`LATEST_ESTIMATED_DELIVERY_DATE`,10),NULL), "
                + "o.`REQ_DELIVERY_DATE`=IFNULL(LEFT(t.`REQ_DELIVERY_DATE`,10),NULL), "
                + "o.`AGREED_DELIVERY_DATE`=IFNULL(LEFT(t.`REVISED_AGREED_DELIVERY_DATE`,10),NULL), "
                + "o.`SUPPLIER_NAME`=t.`ITEM_SUPPLIER_NAME`, "
                + "o.`PRICE`=t.`UNIT_PRICE`, "
                + "o.`SHIPPING_COST`=COALESCE(t.`TOTAL_ACTUAL_FREIGHT_COST`,t.`FREIGHT_ESTIMATE`,t.`SHIPPING_CHARGES`), "
                + "o.`SHIP_BY`=t.`CARRIER_SERVICE_CODE`, "
...
//                + "o.`FLAG`=1;";
        ...
        sql = "UPDATE tmp_erp_shipment t "
                ...
                + "s.`ACTUAL_SHIPMENT_DATE`=IF(t.`ACTUAL_SHIPMENT_DATE` IS NOT NULL AND t.`ACTUAL_SHIPMENT_DATE` != '',DATE_FORMAT(t.`ACTUAL_SHIPMENT_DATE`,'%Y-%m-%d %H:%i:%s'),NULL), "
                + "s.`ACTUAL_DELIVERY_DATE`=IF(t.`ACTUAL_DELIVERY_DATE` IS NOT NULL AND t.`ACTUAL_DELIVERY_DATE` != '',DATE_FORMAT(t.`ACTUAL_DELIVERY_DATE`,'%Y-%m-%d %H:%i:%s'),NULL), "
