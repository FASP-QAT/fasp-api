...
public class ReportDaoImpl implements ReportDao {
    ...
    public List<GlobalConsumptionOutput> getGlobalConsumption(GlobalConsumptionInput gc, CustomUserDetails curUser) {
        ...
        params.put("versionId", (gc.getProgramIds().length > 1 ? -1 : gc.getVersionId()));
...
// Report no 21
    ...
    public List<BudgetReportOutput> getBudgetReport(BudgetReportInput br, CustomUserDetails curUser) {
        ...
        StringBuilder sb = new StringBuilder("SELECT "
                ...
                + "    (b.BUDGET_AMT * b.CONVERSION_RATE_TO_USD) `BUDGET_AMT`, SUM(IFNULL(stc.PLANNED_BUDGET,0)) `PLANNED_BUDGET_AMT`, SUM(IFNULL(stc.ORDERED_BUDGET,0)) `ORDERED_BUDGET_AMT`, b.START_DATE, b.STOP_DATE "
                + "FROM vw_budget b "
                ...
                + "GROUP BY s.PROGRAM_ID,st.BUDGET_ID) stc ON stc.BUDGET_ID=b.BUDGET_ID "
                + "WHERE "
                + "	TRUE AND b.ACTIVE "
