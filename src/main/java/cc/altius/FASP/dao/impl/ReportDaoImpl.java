...
"                     LEFT JOIN ap_label irpu_label ON irpu_label.`LABEL_ID`=pu.`LABEL_ID`  WHERE rc.REALM_ID=:realmId ");
    ...
    public List<CostOfInventoryOutput> getCostOfInventory(CostOfInventoryInput cii, CustomUserDetails curUser) {
        ...
        params.put("regionList", cii.getRegionList());
        params.put("planningUnitList", cii.getPlanningUnitList());
        ...
        return this.namedParameterJdbcTemplate.query("CALL costOfInventory(:programId, :regionList, :planningUnitList, :versionId, :dt, :includePlannedShipments)", new CostOfInventoryRowMapper());
    }
