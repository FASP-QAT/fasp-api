...
public class ProgramDaoImpl implements ProgramDao {
    ...
    public String sqlListStringForProgramPlanningUnit = "SELECT ppu.PROGRAM_PLANNING_UNIT_ID,   "
            ...
            + "    ppu.PLAN_BASED_ON, ppu.MIN_QTY, ppu.DISTRIBUTION_LEAD_TIME, ppu.FORECAST_ERROR_THRESHOLD, ppu.NOTES, "
    ...
    public int saveProgramPlanningUnit(ProgramPlanningUnit[] programPlanningUnits, CustomUserDetails curUser) throws AccessControlFailedException {
        ...
        for (ProgramPlanningUnit ppu : programPlanningUnits) {
            if (ppu.getProgramPlanningUnitId() == 0) {
                ...
                params.put("FORECAST_ERROR_THRESHOLD", ppu.getForecastErrorThreshold());
                ...
                params.put("forecastErrorThreshold", ppu.getForecastErrorThreshold());
                params.put("notes", ppu.getNotes());
                params.put("curDate", curDate);
                params.put("curUser", curUser.getUserId());
                                    ...
                                    try {
                                        ...
                                        switch (es.getStatus()) {
                                            ...
                                            case 2: // Insert
                                                ...
                                                params.put("BATCH_NO", (es.isAutoGenerated() ? Batch.getAutoGeneratedBatchNo(erpOrderDTO.getShProgramId(), erpOrderDTO.getEoPlanningUnitId(), erpOrderDTO.getCalculatedExpiryDate()) : es.getBatchNo()));
...
//                                    logger.info("ERP Linking : Pushed into shipmentBatchTrans with Qty " + es.getBatchQty());
                                ...
                                params.put("PROGRAM_ID", erpOrderDTO.getShProgramId());
                                params.put("PLANNING_UNIT_ID", erpOrderDTO.getEoPlanningUnitId());
                                params.put("BATCH_NO", Batch.getAutoGeneratedBatchNo(erpOrderDTO.getShProgramId(), erpOrderDTO.getEoPlanningUnitId(), erpOrderDTO.getCalculatedExpiryDate()));
...
//        System.out.println(params);
    ...
    public List<DatasetPlanningUnit> getDatasetPlanningUnitList(int programId, int versionId) {
        String sqlString = "SELECT "
                ...
                + "    dpus.REGION_ID, GROUP_CONCAT(DISTINCT CONCAT(dpustl.TREE_ID, '~', dpustl.SCENARIO_ID)) `TREE_AND_SCENARIO`, dpus.CONSUMPTION_EXTRAPOLATION_ID, SUM(COALESCE(mom.CALCULATED_MMD_VALUE, fced.AMOUNT)) `TOTAL_FORECAST`, dpus.NOTES `SELECTED_NOTES`, "
