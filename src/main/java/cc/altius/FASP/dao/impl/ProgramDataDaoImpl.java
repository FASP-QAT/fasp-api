...
//            }
        ...
        params.clear();
        // Flag the rows for changed records
        // Add condition for null dates
        sqlString = "UPDATE tmp_shipment ts "
                + "LEFT JOIN rm_shipment s ON ts.SHIPMENT_ID=s.SHIPMENT_ID "
                + "LEFT JOIN rm_shipment_trans st ON ts.SHIPMENT_ID=st.SHIPMENT_ID AND ts.VERSION_ID=st.VERSION_ID "
                + "SET "
                + " ts.CHANGED=1 "
                + "WHERE "
                ...
                + "ts.PARENT_LINKED_SHIPMENT_ID!=st.PARENT_LINKED_SHIPMENT_ID OR "
                ...
                + "ts.REALM_COUNTRY_PLANNING_UNIT_ID!=st.REALM_COUNTRY_PLANNING_UNIT_ID OR "
                ...
                + "ts.SHIPMENT_RCPU_QTY!=st.SHIPMENT_RCPU_QTY OR "
                + "ts.RATE!=st.RATE OR "
                + "ts.PRODUCT_COST!=st.PRODUCT_COST OR "
                + "ts.SHIPMENT_MODE!=st.SHIPMENT_MODE OR "
                + "ts.FREIGHT_COST!=st.FREIGHT_COST OR "
                + "ts.PLANNED_DATE!=st.PLANNED_DATE OR (ts.PLANNED_DATE IS NULL AND st.PLANNED_DATE IS NOT NULL) OR (ts.PLANNED_DATE IS NOT NULL AND st.PLANNED_DATE IS NULL) OR "
                + "ts.SUBMITTED_DATE!=st.SUBMITTED_DATE OR (ts.SUBMITTED_DATE IS NULL AND st.SUBMITTED_DATE IS NOT NULL) OR (ts.SUBMITTED_DATE IS NOT NULL AND st.SUBMITTED_DATE IS NULL) OR "
                + "ts.APPROVED_DATE!=st.APPROVED_DATE OR (ts.APPROVED_DATE IS NULL AND st.APPROVED_DATE IS NOT NULL) OR (ts.APPROVED_DATE IS NOT NULL AND st.APPROVED_DATE IS NULL) OR "
                + "ts.SHIPPED_DATE!=st.SHIPPED_DATE OR (ts.SHIPPED_DATE IS NULL AND st.SHIPPED_DATE IS NOT NULL) OR (ts.SHIPPED_DATE IS NOT NULL AND st.SHIPPED_DATE IS NULL) OR "
                + "ts.ARRIVED_DATE!=st.ARRIVED_DATE OR (ts.ARRIVED_DATE IS NULL AND st.ARRIVED_DATE IS NOT NULL) OR (ts.ARRIVED_DATE IS NOT NULL AND st.ARRIVED_DATE IS NULL) OR "
                + "ts.RECEIVED_DATE!=st.RECEIVED_DATE OR (ts.RECEIVED_DATE IS NULL AND st.RECEIVED_DATE IS NOT NULL) OR (ts.RECEIVED_DATE IS NOT NULL AND st.RECEIVED_DATE IS NULL) OR "
