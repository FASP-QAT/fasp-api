...
//                }
        ...
        sqlString = "SELECT "
                ...
                + "    ADDDATE(CONCAT(LEFT(LEAST(st.EXPECTED_DELIVERY_DATE, st.RECEIVED_DATE),7),'-01'), INTERVAL ppu.SHELF_LIFE MONTH) `PROJECTED_EXPIRY_DATE`, "
                + "    st.PLANNING_UNIT_ID, st.`SHIPMENT_RCPU_QTY`, s.CREATED_DATE "
                + "FROM vw_program p "
        ...
        sqlString = "SELECT "
                ...
                + "    st.PLANNING_UNIT_ID, st.SHIPMENT_RCPU_QTY, SUM(stbi.BATCH_SHIPMENT_QTY) `BATCH_QTY`, st.SHIPMENT_RCPU_QTY-SUM(stbi.BATCH_SHIPMENT_QTY) `TO_CREATE_BATCH_QTY`, s.`CREATED_DATE` "
                + "FROM vw_program p "
                + "LEFT JOIN rm_shipment s ON p.PROGRAM_ID=s.PROGRAM_ID "
                + "LEFT JOIN rm_shipment_trans st ON s.SHIPMENT_ID=st.SHIPMENT_ID AND s.MAX_VERSION_ID=st.VERSION_ID "
                + "LEFT JOIN rm_shipment_trans_batch_info stbi ON st.SHIPMENT_TRANS_ID=stbi.SHIPMENT_TRANS_ID "
        ...
        sqlString = "SELECT "
                ...
                + "    ADDDATE(CONCAT(LEFT(LEAST(st.EXPECTED_DELIVERY_DATE, st.RECEIVED_DATE),7),'-01'), INTERVAL ppu.SHELF_LIFE MONTH) `PROJECTED_EXPIRY_DATE`, "
                + "    st.PLANNING_UNIT_ID, st.SHIPMENT_RCPU_QTY `TO_CREATE_BATCH_QTY`, s.CREATED_DATE, bi.BATCH_NO "
                + "FROM vw_program p "
                + "LEFT JOIN rm_shipment s ON p.PROGRAM_ID=s.PROGRAM_ID "
                + "LEFT JOIN rm_shipment_trans st ON s.SHIPMENT_ID=st.SHIPMENT_ID AND s.MAX_VERSION_ID=st.VERSION_ID "
                + "LEFT JOIN rm_shipment_trans_batch_info stbi ON st.SHIPMENT_TRANS_ID=stbi.SHIPMENT_TRANS_ID "
