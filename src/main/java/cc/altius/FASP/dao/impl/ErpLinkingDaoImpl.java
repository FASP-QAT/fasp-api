...
//        }
    ...
    public List<ShipmentLinkingOutput> getNotLinkedErpShipments(NotLinkedErpShipmentsInputTab1 input, CustomUserDetails curUser) {
        StringBuilder sqlStringBuilder = new StringBuilder(""
                ...
                + "LEFT JOIN rm_shipment_linking sl ON sl.RO_NO=e.RO_NO and sl.RO_PRIME_LINE_NO=e.RO_PRIME_LINE_NO   "
                + "LEFT JOIN rm_shipment_linking_trans slt ON slt.SHIPMENT_LINKING_ID=sl.SHIPMENT_LINKING_ID AND slt.VERSION_ID=sl.MAX_VERSION_ID AND slt.ACTIVE   "
                + "LEFT JOIN rm_shipment sh ON sl.CHILD_SHIPMENT_ID=s.SHIPMENT_ID "
    ...
    public List<ShipmentLinkingOutput> getLinkedQatShipments(int programId, int versionId, String[] planningUnitIds, CustomUserDetails curUser) {
        StringBuilder sqlStringBuilder = new StringBuilder("SELECT "
                ...
                + "                    e.PRICE, e.SHIPPING_COST, "
                + "                    ss.SHIPMENT_STATUS_ID, ss.LABEL_ID `SS_LABEL_ID`, ss.LABEL_EN `SS_LABEL_EN`, ss.LABEL_FR `SS_LABEL_FR`, ss.LABEL_SP `SS_LABEL_SP`, ss.LABEL_PR  `SS_LABEL_PR`, "
                + "                    sl.PARENT_SHIPMENT_ID, s2t.PARENT_LINKED_SHIPMENT_ID, sl.CHILD_SHIPMENT_ID, s2t.NOTES, e.SHIP_BY, slt.`CONVERSION_FACTOR` CONVERSION_FACTOR,null `TRACER_CATEGORY_ID`                      "
    ...
    public List<ShipmentLinkedToOtherProgramOutput> getShipmentLinkedToOtherProgram(ShipmentLinkedToOtherProgramInput shipmentInput, CustomUserDetails curUser) {
        ...
        sqlString = "SELECT "
                ...
                + "    sl.RO_NO, sl.RO_PRIME_LINE_NO, sl.PARENT_SHIPMENT_ID, st.PARENT_LINKED_SHIPMENT_ID, slt.CONVERSION_FACTOR, "
                ...
                + "LEFT JOIN rm_shipment_trans st ON s.SHIPMENT_ID=st.SHIPMENT_ID AND s.MAX_VERISON_ID=st.VERSION_ID "
