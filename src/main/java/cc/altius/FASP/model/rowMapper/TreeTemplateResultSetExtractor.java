 ...
 * @author akil
 */
public class TreeTemplateResultSetExtractor implements ResultSetExtractor<TreeTemplate> {

    @Override
    public TreeTemplate extractData(ResultSet rs) throws SQLException, DataAccessException {
        TreeTemplate tt = new TreeTemplate();
        boolean isFirst = true;
        while (rs.next()) {
            if (isFirst) {
                tt.setTreeTemplateId(rs.getInt("TREE_TEMPLATE_ID"));
                // Basic
