...
public class ProblemReportResultSetExtractor implements ResultSetExtractor<List<ProblemReport>> {
    ...
    public List<ProblemReport> extractData(ResultSet rs) throws SQLException, DataAccessException {
        ...
        while (rs.next()) {
            ...
            if (idx == -1) {
                pr.setProgram(new SimpleCodeObject(rs.getInt("PROGRAM_ID"), new LabelRowMapper("PROGRAM_").mapRow(rs, idx), rs.getString("PROGRAM_CODE")));
                pr.setVersionId(rs.getInt("VERSION_ID"));
                pr.setData1(rs.getString("DATA1"));
                pr.setData2(rs.getString("DATA2"));
                pr.setData3(rs.getString("DATA3"));
                pr.setData4(rs.getString("DATA4"));
                pr.setData5(rs.getString("DATA5"));
                pr.setProblemStatus(new SimpleObject(rs.getInt("PROBLEM_STATUS_ID"), new LabelRowMapper("PROBLEM_STATUS_").mapRow(rs, idx)));
                pr.setProblemType(new SimpleObject(rs.getInt("PROBLEM_TYPE_ID"), new LabelRowMapper("PROBLEM_TYPE_").mapRow(rs, idx)));
                pr.setRealmProblem(new RealmProblemRowMapper("RP_", true).mapRow(rs, idx));
            ...
            prt.setProblemStatus(new SimpleObject(rs.getInt("PROBLEM_STATUS_TRANS_ID"), new LabelRowMapper("PROBLEM_STATUS_TRANS_").mapRow(rs, idx)));
