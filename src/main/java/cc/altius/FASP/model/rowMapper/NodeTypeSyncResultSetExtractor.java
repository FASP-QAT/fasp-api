...
public class NodeTypeSyncResultSetExtractor implements ResultSetExtractor<List<NodeTypeSync>> {
    ...
    public List<NodeTypeSync> extractData(ResultSet rs) throws SQLException, DataAccessException {
        ...
        while (rs.next()) {
            NodeTypeSync nt = new NodeTypeSync(rs.getInt("ID"), new LabelRowMapper().mapRow(rs, 1), rs.getBoolean("MODELING_ALLOWED"), rs.getBoolean("EXTRAPOLATION_ALLOWED"), rs.getBoolean("TREE_TEMPLATE_ALLOWED"), rs.getBoolean("FORECAST_TREE_ALLOWED"));
