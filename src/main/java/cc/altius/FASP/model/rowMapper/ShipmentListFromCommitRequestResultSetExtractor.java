 ...
 * @author akil
 */
public class ShipmentListFromCommitRequestResultSetExtractor implements ResultSetExtractor<List<Shipment>> {
    ...
    public List<Shipment> extractData(ResultSet rs) throws SQLException, DataAccessException {
        ...
        while (rs.next()) {
            ...
            if (!rs.wasNull()) {
                ...
                sb.setShipmentQty(rs.getLong("BATCH_SHIPMENT_QTY"));
                s.getBatchInfoList().add(sb);
            }
            oldId = curId;
        }
        return sList;
