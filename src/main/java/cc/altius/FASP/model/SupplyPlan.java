...
public class SupplyPlan implements Serializable {
    ...
    public int getPrevClosingBalanceWtbdps(int planningUnitId, int batchId, String transDate) {
        SupplyPlanDate s1 = new SupplyPlanDate(planningUnitId, transDate);
        int idx1 = this.supplyPlanDateList.indexOf(s1);
        if (idx1 == -1) {
            return 0;
            ...
            if (idx2 == -1) {
                ...
                return this.supplyPlanDateList.get(idx1).getBatchList().get(idx2).getClosingBalanceWtbdps();
            }
        }
    }
