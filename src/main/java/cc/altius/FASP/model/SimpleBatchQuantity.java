/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package cc.altius.FASP.model;

import cc.altius.FASP.framework.JsonDateDeserializer;
import cc.altius.FASP.framework.JsonDateSerializer;
import com.fasterxml.jackson.annotation.JsonView;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonSerialize;
import java.io.Serializable;
import java.util.Date;

/**
 *
 * @author akil
 */
public class SimpleBatchQuantity implements Serializable {

    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private int batchId;
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private String batchNo;
    @JsonDeserialize(using = JsonDateDeserializer.class)
    @JsonSerialize(using = JsonDateSerializer.class)
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private Date expiryDate;
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private boolean autoGenerated;
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private long qty;
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private long qtyWps;
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private long expiredQty;
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private long expiredQtyWps;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long consumptionQty;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long stockQty;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long adjustmentQty;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long shipmentQty;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long shipmentQtyWps;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long unallocatedQty;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private Long unallocatedQtyWps;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private long openingBalance;
    @JsonView({Views.InternalView.class, Views.ReportView.class})
    private long openingBalanceWps;

    @JsonDeserialize(using = JsonDateDeserializer.class)
    @JsonSerialize(using = JsonDateSerializer.class)
    @JsonView({Views.ArtmisView.class, Views.InternalView.class, Views.ReportView.class})
    private Date createdDate;

    public SimpleBatchQuantity() {
    }

    public SimpleBatchQuantity(int batchId, String batchNo, Date expiryDate, boolean autoGenerated, long qty, long qtyWps, long expiredQty, long expiredQtyWps, Date createdDate, Long shipmentQty, Long shipmentQtyWps) {
        this.batchId = batchId;
        this.batchNo = batchNo;
        this.expiryDate = expiryDate;
        this.autoGenerated = autoGenerated;
        this.qty = qty;
        this.qtyWps = qtyWps;
        this.expiredQty = expiredQty;
        this.expiredQtyWps = expiredQtyWps;
        this.createdDate = createdDate;
        this.stockQty = stockQty;
        this.shipmentQty = shipmentQty;
        this.shipmentQtyWps = shipmentQtyWps;
    }

    public int getBatchId() {
        return batchId;
    }

    public void setBatchId(int batchId) {
        this.batchId = batchId;
    }

    public String getBatchNo() {
        return batchNo;
    }

    public void setBatchNo(String batchNo) {
        this.batchNo = batchNo;
    }

    public Date getExpiryDate() {
        return expiryDate;
    }

    public void setExpiryDate(Date expiryDate) {
        this.expiryDate = expiryDate;
    }

    public boolean isAutoGenerated() {
        return autoGenerated;
    }

    public void setAutoGenerated(boolean autoGenerated) {
        this.autoGenerated = autoGenerated;
    }

    public long getQty() {
        return qty;
    }

    public void setQty(long qty) {
        this.qty = qty;
    }

    public long getQtyWps() {
        return qtyWps;
    }

    public void setQtyWps(long qtyWps) {
        this.qtyWps = qtyWps;
    }

    public long getExpiredQty() {
        return expiredQty;
    }

    public void setExpiredQty(long expiredQty) {
        this.expiredQty = expiredQty;
    }

    public long getExpiredQtyWps() {
        return expiredQtyWps;
    }

    public void setExpiredQtyWps(long expiredQtyWps) {
        this.expiredQtyWps = expiredQtyWps;
    }

    public Date getCreatedDate() {
        return createdDate;
    }

    public void setCreatedDate(Date createdDate) {
        this.createdDate = createdDate;
    }

    public Long getConsumptionQty() {
        return consumptionQty;
    }

    public void setConsumptionQty(Long consumptionQty) {
        this.consumptionQty = consumptionQty;
    }

    public Long getStockQty() {
        return stockQty;
    }

    public void setStockQty(Long stockQty) {
        this.stockQty = stockQty;
    }

    public Long getAdjustmentQty() {
        return adjustmentQty;
    }

    public void setAdjustmentQty(Long adjustmentQty) {
        this.adjustmentQty = adjustmentQty;
    }

    public Long getShipmentQty() {
        return shipmentQty;
    }

    public void setShipmentQty(Long shipmentQty) {
        this.shipmentQty = shipmentQty;
    }

    public Long getUnallocatedQty() {
        return unallocatedQty;
    }

    public void setUnallocatedQty(Long unallocatedQty) {
        this.unallocatedQty = unallocatedQty;
    }

    public long getOpeningBalance() {
        return openingBalance;
    }

    public void setOpeningBalance(long openingBalance) {
        this.openingBalance = openingBalance;
    }

    public Long getShipmentQtyWps() {
        return shipmentQtyWps;
    }

    public void setShipmentQtyWps(Long shipmentQtyWps) {
        this.shipmentQtyWps = shipmentQtyWps;
    }

    public Long getUnallocatedQtyWps() {
        return unallocatedQtyWps;
    }

    public void setUnallocatedQtyWps(Long unallocatedQtyWps) {
        this.unallocatedQtyWps = unallocatedQtyWps;
    }

    public long getOpeningBalanceWps() {
        return openingBalanceWps;
    }

    public void setOpeningBalanceWps(long openingBalanceWps) {
        this.openingBalanceWps = openingBalanceWps;
    }

}
