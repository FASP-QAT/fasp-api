...
public class DashboardTopRowMapper implements RowMapper<DashboardTop> {
    ...
    public DashboardTop mapRow(ResultSet rs, int rowNum) throws SQLException {
        return new DashboardTop(
                ...
                rs.getInt("COUNT_OF_OPEN_PROBLEM"), rs.getInt("VERSION_ID"), 
                rs.getObject("LATEST_FINAL_VERSION_ID") != null ? ((Number) rs.getObject("LATEST_FINAL_VERSION_ID")).intValue() : null, 
                new SimpleObject(rs.getInt("LATEST_FINAL_VERSION_STATUS_ID"), new LabelRowMapper("LATEST_FINAL_VS_").mapRow(rs, rowNum)), rs.getDate("LATEST_FINAL_VERSION_LAST_MODIFIED_DATE")
        );
