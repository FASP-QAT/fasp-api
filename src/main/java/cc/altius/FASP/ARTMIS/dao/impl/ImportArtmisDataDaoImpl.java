        ...
        if (fr.read() == -1) {
            ...
            for (int temp2 = 0; temp2 < nList1.getLength(); temp2++) {
                ...
                if (nNode1.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    map.put("REVISED_AGREED_DELIVERY_DATE", dataRecordElement.getElementsByTagName("revised_agreed_delivery_date").item(0).getTextContent().isBlank() ? dataRecordElement.getElementsByTagName("revised_agreed_delivery_date").item(0).getTextContent() : dataRecordElement.getElementsByTagName("revised_agreed_delivery_date").item(0).getTextContent());
...
//            map.clear();
            x = 0;
            sqlString = "INSERT INTO tmp_erp_shipment (`KN_SHIPMENT_NUMBER`, `ORDER_NO`, `PRIME_LINE_NO`, `BATCH_NO`, `ITEM_ID`, "
                    ...
                    + "VALUES (:KN_SHIPMENT_NUMBER, :ORDER_NO, :PRIME_LINE_NO, :BATCH_NO, :ITEM_ID, "
                    + ":EXPIRATION_DATE, :SHIPPED_QUANTITY, :DELIVERED_QUANTITY, :STATUS_NAME, :EXTERNAL_STATUS_STAGE, "
                    + ":ACTUAL_SHIPMENT_DATE, :ACTUAL_DELIVERY_DATE)";
            ...
            sqlString = "INSERT IGNORE INTO rm_erp_order (`RO_NO`, `RO_PRIME_LINE_NO`, `ORDER_NO`, `PRIME_LINE_NO`, `ORDER_TYPE`, "
                    ...
                    + "`ORDERED_DATE`, `CURRENT_ESTIMATED_DELIVERY_DATE`, `REQ_DELIVERY_DATE`, `AGREED_DELIVERY_DATE`, `SUPPLIER_NAME`, "
                    ...
                    + " SELECT RO_NO, RO_PRIME_LINE_NO, ORDER_NO, PRIME_LINE_NO, ORDER_TYPE_IND,"
                    + " PARENT_RO, DATE_FORMAT(PARENT_ORDER_ENTRY_DATE,'%Y-%m-%d %H:%i:%s'), LEFT(ITEM_ID, 12), IF(LENGTH(ITEM_ID)=15,ITEM_ID,NULL), ORDERED_QTY,"
                    + " IFNULL(DATE_FORMAT(PO_RELEASED_FOR_FULFILLMENT_DATE,'%Y-%m-%d'),NULL), IFNULL(DATE_FORMAT(LATEST_ESTIMATED_DELIVERY_DATE,'%Y-%m-%d'),NULL), IFNULL(DATE_FORMAT(REQ_DELIVERY_DATE,'%Y-%m-%d'),NULL), IFNULL(DATE_FORMAT(REVISED_AGREED_DELIVERY_DATE,'%Y-%m-%d'),NULL), ITEM_SUPPLIER_NAME, "
                    + " UNIT_PRICE,COALESCE(TOTAL_ACTUAL_FREIGHT_COST,FREIGHT_ESTIMATE,SHIPPING_CHARGES), CARRIER_SERVICE_CODE, RECIPIENT_NAME, RECIPIENT_COUNTRY,"
                    + " STATUS_NAME, 1, :curDate, 1, NULL, PROGRAM_ID, SHIPMENT_ID, DATE_FORMAT(ORDER_ENTRY_DATE,'%Y-%m-%d') "
            ...
            sqlString = "UPDATE tmp_erp_shipment t "
                    + "LEFT JOIN rm_erp_shipment s ON t.`KN_SHIPMENT_NUMBER`=s.`KN_SHIPMENT_NO` AND t.`ORDER_NUMBER`=s.`ORDER_NO` AND t.`PRIME_LINE_NO`=s.`PRIME_LINE_NO` AND t.`BATCH_NO`=s.`BATCH_NO` "
                    + "SET "
                    + " s.`FLAG`=1, "
            ...
            sqlString = "INSERT IGNORE INTO rm_erp_shipment (KN_SHIPMENT_NO, ORDER_NO, PRIME_LINE_NO, BATCH_NO, "
                    ...
                    + "ACTUAL_DELIVERED_DATE, STATUS, LAST_MODIFIED_DATE, FLAG)"
                    + "SELECT KN_SHIPMENT_NUMBER, ORDER_NO, PRIME_LINE_NO, BATCH_NO,"
                    + "DATE_FORMAT(EXPIRATION_DATE,'%Y-%m-%d %H:%i:%s'), IF(LENGTH(ITEM_ID)=15,ITEM_ID, null), SHIPPED_QUANTITY, DELIVERED_QUANTITY, DATE_FORMAT(ACTUAL_SHIPMENT_DATE,'%Y-%m-%d %H:%i:%s'), "
                    + "DATE_FORMAT(ACTUAL_DELIVERY_DATE,'%Y-%m-%d %H:%i:%s'), EXTERNAL_STATUS_STAGE, :curDate,  "
