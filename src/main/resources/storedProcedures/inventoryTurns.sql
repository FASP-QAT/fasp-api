CREATE DEFINER=`faspUser`@`localhost` PROCEDURE `inventoryTurns`(VAR_START_DATE DATE, VAR_VIEW_BY INT, VAR_REALM_COUNTRY_IDS TEXT, VAR_PRODUCT_CATEGORY_IDS TEXT, VAR_INCLUDE_PLANNED_SHIPMENTS BOOLEAN)
BEGIN
    -- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    -- Report no 9
    -- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    DECLARE program_cursor CURSOR FOR 
        SELECT p.PROGRAM_ID, MAX(pv.VERSION_ID) VERSION_ID
           ...
           WHERE FIND_IN_SET(p.REALM_COUNTRY_ID, VAR_REALM_COUNTRY_IDS) AND p.ACTIVE 
           GROUP BY p.PROGRAM_ID
           HAVING MAX(pv.VERSION_ID) IS NOT NULL
           ORDER BY p.REALM_COUNTRY_ID, p.PROGRAM_ID;
    ...
    IF @viewBy = 1 THEN
        SELECT 
            ...
            it.PROGRAM_ID, p.LABEL_ID `P_LABEL_ID`, p.LABEL_EN `P_LABEL_EN`, p.LABEL_FR `P_LABEL_FR`, p.LABEL_SP `P_LABEL_SP`, p.LABEL_PR `P_LABEL_PR`, 
            ...
            it.AVG_STOCK,
            it.NO_OF_MONTHS,
            it.TOTAL_CONSUMPTION/it.AVG_STOCK `INVENTORY_TURNS`
