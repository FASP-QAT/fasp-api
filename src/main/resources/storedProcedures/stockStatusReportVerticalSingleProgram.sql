CREATE DEFINER=`faspUser`@`localhost` PROCEDURE `stockStatusReportVerticalSingleProgram`(VAR_START_DATE DATE, VAR_STOP_DATE DATE, VAR_PROGRAM_IDS TEXT, VAR_VERSION_ID INT, VAR_REPORTING_UNIT_IDS, VAR_VIEW_BY INT(10), VAR_EQUIVALENCY_UNIT_ID INT(10)
BEGIN
    -- %%%%%%%%%%%%%%%%%%%%%
    -- Report no 16
    -- %%%%%%%%%%%%%%%%%%%%%
...
--      `REPORTING_UNIT_ID` int unsigned NOT NULL,
...
--     IF @equivalencyUnitId != 0 THEN
--         SELECT eu.EQUIVALENCY_UNIT_ID, eu.LABEL_ID, eu.LABEL_EN, eu.LABEL_FR, eu.LABEL_SP, eu.LABEL_PR, eu.MULTPLIER INTO @ruId, @ruLabelId, @ruLabelEn, @ruLabelFr, @ruLabelSp, @ruLabelFr, @multiplier FROM vw_equivalency_unit eu WHERE eu.EQUIVALENCY_UNIT_ID=@equivalencyUnitId;
--         IF @viewBy = 1 THEN
--             SET @planningUnitId = @reportingUnitId;
--         ELSE 
--             SELECT rcpu.PLANNING_UNIT_ID INTO @planningUnitId FROM vw_program p LEFT JOIN rm_realm_country_planning_unit rcpu ON p.REALM_COUNTRY_ID=rcpu.REALM_COUNTRY_ID WHERE p.PROGRAM_ID=@programId AND rcpu.REALM_COUNTRY_PLANNING_UNIT_ID=@reportingUnitId;
--         END IF;
--     ELSEIF @viewBy = 1 THEN
--         SELECT p.PLANNING_UNIT_ID, p.LABEL_ID, p.LABEL_EN, p.LABEL_FR, p.LABEL_SP, p.LABEL_PR, 1 INTO @ruId, @ruLabelId, @ruLabelEn, @ruLabelFr, @ruLabelSp, @ruLabelFr, @multiplier FROM vw_planning_unit pu WHERE pu.PLANNING_UNIT_ID=@reportingUnitId;
--     ELSE 
--         SELECT rcpu.PLANNING_UNIT_ID, rcpu.REALM_COUNTRY_PLANNING_UNIT_ID, rcpu.LABEL_ID, rcpu.LABEL_EN, rcpu.LABEL_FR, rcpu.LABEL_SP, rcpu.LABEL_PR, rcpu.MULTIPLIER INTO @planningUnitId, @ruId, @ruLabelId, @ruLabelEn, @ruLabelFr, @ruLabelSp, @ruLabelFr, @multiplier FROM vw_program p LEFT JOIN rm_realm_country_planning_unit rcpu ON p.REALM_COUNTRY_ID=rcpu.REALM_COUNTRY_ID WHERE p.PROGRAM_ID=@programId AND rcpu.REALM_COUNTRY_PLANNING_UNIT_ID=@reportingUnitId;
--     END IF;
    

--     SELECT @planningUnitId, @ruId, @ruLabelEn, @multiplier;
