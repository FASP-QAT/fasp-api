CREATE TABLE `fasp`.`rm_organisation_type` (
...
INSERT INTO `fasp`.`ap_label_source` (`SOURCE_ID`, `SOURCE_DESC`) VALUES ('37', 'rm_organisation_type');
...
ALTER TABLE `fasp`.`rm_organisation` ADD COLUMN `ORGANISATION_TYPE_ID` INT(10) UNSIGNED NOT NULL DEFAULT 1 AFTER `ORGANISATION_CODE`, ADD INDEX `fk_rm_organisation_organisationTypeId_idx` (`ORGANISATION_TYPE_ID` ASC);
ALTER TABLE `fasp`.`rm_organisation` ADD CONSTRAINT `fk_rm_organisation_organisationTypeId` FOREIGN KEY (`ORGANISATION_TYPE_ID`) REFERENCES `fasp`.`rm_organisation_type` (`ORGANISATION_TYPE_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION;
