...
BEGIN 
    ...
    SELECT 
        ...
        SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths MONTH) AND mn.MONTH, c1.FORECAST_CONSUMPTION, null)) SUM_OF_FORECAST,
    ...
    ) AS c1 ON c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths MONTH) AND mn.MONTH AND pr.REGION_ID=c1.REGION_ID AND ((@viewBy=1 AND c1.PLANNING_UNIT_ID=pu.PLANNING_UNIT_ID) OR (@viewBy=2 AND c1.FORECASTING_UNIT_ID=fu.FORECASTING_UNIT_ID))
