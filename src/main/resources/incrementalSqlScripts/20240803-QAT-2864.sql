ALTER TABLE `fasp`.`rm_shipment_trans` ADD COLUMN `AUTO_GENERATED_FREIGHT` TINYINT UNSIGNED NULL AFTER `PARENT_LINKED_SHIPMENT_ID`;

UPDATE rm_shipment_trans st SET st.AUTO_GENERATED_FREIGHT=0 WHERE st.ERP_FLAG=1 AND st.AUTO_GENERATED_FREIGHT IS NULL;
UPDATE rm_shipment_trans st SET st.AUTO_GENERATED_FREIGHT=0 WHERE st.ACTIVE=0 AND st.AUTO_GENERATED_FREIGHT IS NULL;

UPDATE rm_shipment_trans st LEFT JOIN (
SELECT 
    st.SHIPMENT_TRANS_ID, st.PRODUCT_COST, 
    s.PROGRAM_ID, st.PLANNING_UNIT_ID, st.PROCUREMENT_AGENT_ID, st.SHIPMENT_MODE, st.FREIGHT_COST,
    COALESCE(
        CASE 
            WHEN st.SHIPMENT_MODE='Air' THEN ppupa1.`AIR_FREIGHT_PERC` 
            WHEN st.SHIPMENT_MODE='Sea' THEN ppupa1.`SEA_FREIGHT_PERC` 
            WHEN st.SHIPMENT_MODE='Road' THEN ppupa1.`ROAD_FREIGHT_PERC` 
            ELSE NULL 
        END, 
        CASE 
            WHEN st.SHIPMENT_MODE='Air' THEN ppupa2.`AIR_FREIGHT_PERC` 
            WHEN st.SHIPMENT_MODE='Sea' THEN ppupa2.`SEA_FREIGHT_PERC` 
            WHEN st.SHIPMENT_MODE='Road' THEN ppupa2.`ROAD_FREIGHT_PERC` 
            ELSE NULL 
        END, 
        CASE 
            WHEN st.SHIPMENT_MODE='Air' THEN p.`AIR_FREIGHT_PERC` 
            WHEN st.SHIPMENT_MODE='Sea' THEN p.`SEA_FREIGHT_PERC` 
            WHEN st.SHIPMENT_MODE='Road' THEN p.`ROAD_FREIGHT_PERC` 
            ELSE NULL 
        END
    ) `FREIGHT_PERC`,
    ROUND(COALESCE(
        ...
        CASE 
            ...
            ELSE NULL 
        END
    ) * st.PRODUCT_COST/100,2) `CALCULATED_FREIGHT_COST`
...
LEFT JOIN rm_program_planning_unit_procurement_agent ppupa1 ON ppu.PROGRAM_ID=ppupa1.PROGRAM_ID AND ppu.PLANNING_UNIT_ID=ppupa1.PLANNING_UNIT_ID AND ppupa1.PROCUREMENT_AGENT_ID=st.PROCUREMENT_AGENT_ID
LEFT JOIN rm_program_planning_unit_procurement_agent ppupa2 ON ppu.PROGRAM_ID=ppupa2.PROGRAM_ID AND ppupa2.PLANNING_UNIT_ID IS NULL AND ppupa2.PROCUREMENT_AGENT_ID=st.PROCUREMENT_AGENT_ID
WHERE 
    st.AUTO_GENERATED_FREIGHT IS NULL) ss ON st.SHIPMENT_TRANS_ID=ss.SHIPMENT_TRANS_ID 
    SET st.AUTO_GENERATED_FREIGHT=1 WHERE ss.`CALCULATED_FREIGHT_COST`=ss.`FREIGHT_COST`;
...
WHERE 
    st.AUTO_GENERATED_FREIGHT IS NULL) ss ON st.SHIPMENT_TRANS_ID=ss.SHIPMENT_TRANS_ID 
    SET st.AUTO_GENERATED_FREIGHT=0 WHERE ss.`CALCULATED_FREIGHT_COST`!=ss.`FREIGHT_COST`;
...
DELIMITER $$
USE `fasp`$$
CREATE DEFINER=`faspUser`@`localhost` PROCEDURE `getShipmentDataNew`(PROGRAM_ID INT(10), VERSION_ID INT (10), SHIPMENT_ACTIVE TINYINT(1), PLANNING_UNIT_ACTIVE TINYINT(1), CUT_OFF_DATE DATE)
BEGIN
    ...
    FROM (
        SELECT
            s.SHIPMENT_ID, s.PARENT_SHIPMENT_ID, st.EXPECTED_DELIVERY_DATE, st.PLANNED_DATE, st.SUBMITTED_DATE, st.APPROVED_DATE, st.SHIPPED_DATE, st.ARRIVED_DATE, st.RECEIVED_DATE, st.SHIPMENT_QTY, st.SHIPMENT_RCPU_QTY, pu.MULTIPLIER `CONVERSION_FACTOR`, st.RATE, st.PRODUCT_COST, st.FREIGHT_COST, st.AUTO_GENERATED_FREIGHT, st.SHIPMENT_MODE, s.SUGGESTED_QTY, st.ACCOUNT_FLAG, st.ERP_FLAG, st.ORDER_NO, st.PRIME_LINE_NO, st.VERSION_ID, st.NOTES, st.SHIPMENT_TRANS_ID, st.PARENT_LINKED_SHIPMENT_ID,
            p.PROGRAM_ID, p.LABEL_ID `PROGRAM_LABEL_ID`, p.LABEL_EN `PROGRAM_LABEL_EN`, p.LABEL_FR `PROGRAM_LABEL_FR`, p.LABEL_SP `PROGRAM_LABEL_SP`, p.LABEL_PR `PROGRAM_LABEL_PR`,
            pa.PROCUREMENT_AGENT_ID, pa.PROCUREMENT_AGENT_CODE, pa.`COLOR_HTML_CODE`, pa.LABEL_ID `PROCUREMENT_AGENT_LABEL_ID`, pa.LABEL_EN `PROCUREMENT_AGENT_LABEL_EN`, pa.LABEL_FR `PROCUREMENT_AGENT_LABEL_FR`, pa.LABEL_SP `PROCUREMENT_AGENT_LABEL_SP`, pa.LABEL_PR `PROCUREMENT_AGENT_LABEL_PR`,
            pu.PLANNING_UNIT_ID, pu.LABEL_ID `PLANNING_UNIT_LABEL_ID`, pu.LABEL_EN `PLANNING_UNIT_LABEL_EN`, pu.LABEL_FR `PLANNING_UNIT_LABEL_FR`, pu.LABEL_SP `PLANNING_UNIT_LABEL_SP`, pu.LABEL_PR `PLANNING_UNIT_LABEL_PR`,
            fu.FORECASTING_UNIT_ID, fu.LABEL_ID `FORECASTING_UNIT_LABEL_ID`, fu.LABEL_EN `FORECASTING_UNIT_LABEL_EN`, fu.LABEL_FR `FORECASTING_UNIT_LABEL_FR`, fu.LABEL_SP `FORECASTING_UNIT_LABEL_SP`, fu.LABEL_PR `FORECASTING_UNIT_LABEL_PR`,
...
USE `fasp`;
DROP procedure IF EXISTS `getShipmentDataForSync`;

USE `fasp`;
DROP procedure IF EXISTS `fasp`.`getShipmentDataForSync`;
;
