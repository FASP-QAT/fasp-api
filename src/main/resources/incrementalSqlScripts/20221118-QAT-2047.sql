...
BEGIN
	...
	mn.MONTH, 
        ...
        SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths-1 MONTH) AND mn.MONTH, ABS(c1.FORECAST_CONSUMPTION-IF(@daysOfStockOut,c1.ADJUSTED_ACTUAL_CONSUMPTION,c1.ACTUAL_CONSUMPTION)), null)) SUM_OF_FORECAST_MINUS_ACTUAL,
        --    SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths-1 MONTH) AND mn.MONTH, ABS(c1.FORECAST_CONSUMPTION-c1.ADJUSTED_ACTUAL_CONSUMPTION), null)) SUM_OF_FORECAST_MINUS_ADJUSTED_ACTUAL,
        SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths-1 MONTH) AND mn.MONTH, IF(@daysOfStockOut,c1.ADJUSTED_ACTUAL_CONSUMPTION,c1.ACTUAL_CONSUMPTION), null)) SUM_OF_ACTUAL
        --    SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths-1 MONTH) AND mn.MONTH, c1.ADJUSTED_ACTUAL_CONSUMPTION, null)) SUM_OF_ADJUSTED_ACTUAL,
        --    SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths-1 MONTH) AND mn.MONTH, ABS(c1.FORECAST_CONSUMPTION-IF(@daysOfStockOut, c1.ADJUSTED_ACTUAL_CONSUMPTION, c1.ACTUAL_CONSUMPTION)), null))/SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths MONTH) AND mn.MONTH, IF(@daysOfStockOut, c1.ADJUSTED_ACTUAL_CONSUMPTION, c1.ACTUAL_CONSUMPTION), null)) ERROR_PERC
    FROM mn 
    ...
    LEFT JOIN vw_planning_unit pu ON pu.PLANNING_UNIT_ID=@unitId AND @viewBy=1
    LEFT JOIN vw_forecasting_unit fu ON fu.FORECASTING_UNIT_ID=@unitId AND @viewBy=2
	...
	WHERE
            ...
            AND ((@viewBy=1 AND ct.PLANNING_UNIT_ID=@unitId) OR (@viewBy=2 AND pu.`FORECASTING_UNIT_ID`=@unitId)) GROUP BY ct.CONSUMPTION_DATE, ct.`REGION_ID`
