...
BEGIN
	...
	SET @sql1 = CONCAT(@sql1, "		FROM (");	
		...
		SET @sql1 = CONCAT(@sql1, "		s.SHIPMENT_ID, s.PARENT_SHIPMENT_ID, st.EXPECTED_DELIVERY_DATE, st.PLANNED_DATE, st.SUBMITTED_DATE, st.APPROVED_DATE, st.SHIPPED_DATE, st.ARRIVED_DATE, st.RECEIVED_DATE, st.SHIPMENT_QTY, pu.MULTIPLIER `CONVERSION_FACTOR`, st.RATE, st.PRODUCT_COST, st.FREIGHT_COST, st.SHIPMENT_MODE, s.SUGGESTED_QTY, st.ACCOUNT_FLAG, st.ERP_FLAG, st.ORDER_NO, st.PRIME_LINE_NO, st.VERSION_ID, st.NOTES, st.SHIPMENT_TRANS_ID, 
         ...
         SET @sql1 = CONCAT(@sql1, "		FROM (");
         SET @sql1 = CONCAT(@sql1, "		  SELECT st.SHIPMENT_ID, MAX(st.VERSION_ID) MAX_VERSION_ID FROM rm_shipment s LEFT JOIN rm_shipment_trans st ON s.SHIPMENT_ID=st.SHIPMENT_ID WHERE (@versiONId=-1 OR st.VERSION_ID<=@versiONId) AND s.PROGRAM_ID=@programId GROUP BY st.SHIPMENT_ID");
         SET @sql1 = CONCAT(@sql1, "		) ts ");	
         SET @sql1 = CONCAT(@sql1, "		LEFT JOIN rm_shipment s ON ts.SHIPMENT_ID=s.SHIPMENT_ID
...
LEFT JOIN vw_funding_source fs ON st.FUNDING_SOURCE_ID=fs.FUNDING_SOURCE_ID ");	  
      IF @active THEN
      SET @sql1 = CONCAT(@sql1, "		WHERE st.ACTIVE ");		
      END IF;
      SET @sql1 = CONCAT(@sql1, "		) st  ");	

      SET @sql1 = CONCAT(@sql1, "		LEFT JOIN rm_shipment_trans_batch_info stbi ON st.SHIPMENT_TRANS_ID = stbi.SHIPMENT_TRANS_ID
