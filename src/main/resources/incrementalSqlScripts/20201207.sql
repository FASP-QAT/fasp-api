...
BEGIN
	...
	SET @sqlString = CONCAT(@sqlString, "		SUM(IF(st.SHIPMENT_STATUS_ID IN (1), ((IFNULL(st.FREIGHT_COST,0)+IFNULL(st.PRODUCT_COST,0))*s1.CONVERSION_RATE_TO_USD),0))/1000000 `PLANNED_BUDGET`, "); 
	SET @sqlString = CONCAT(@sqlString, "		SUM(IF(st.SHIPMENT_STATUS_ID IN (3,4,5,6,7,9), ((IFNULL(st.FREIGHT_COST,0)+IFNULL(st.PRODUCT_COST,0))*s1.CONVERSION_RATE_TO_USD),0))/1000000 `ORDERED_BUDGET` "); 
