...
ALTER TABLE `fasp`.`rm_consumption_trans_batch_info`
CHANGE COLUMN `CONSUMPTION_QTY` `CONSUMPTION_QTY` DECIMAL(24,4) NOT NULL ;

ALTER TABLE `fasp`.`rm_inventory_trans` CHANGE `ACTUAL_QTY` `ACTUAL_QTY` BIGINT(20) UNSIGNED NULL COMMENT 'If only an Adjustment Qty is given then this was not an Inventory count but an Adjustment to Inventory. If the Actual Qty is mentioned then the Adjustment Qty was derived', CHANGE `ADJUSTMENT_QTY` `ADJUSTMENT_QTY` BIGINT(20) NULL COMMENT 'If only an Adjustment Qty is given then this was not an Inventory count but an Adjustment to Inventory. If the Actual Qty is mentioned then the Adjustment Qty was derived'; 

ALTER TABLE `fasp`.`rm_inventory_trans_batch_info` CHANGE `ACTUAL_QTY` `ACTUAL_QTY` BIGINT(20) UNSIGNED NULL , CHANGE `ADJUSTMENT_QTY` `ADJUSTMENT_QTY` BIGINT(20) NULL ; 

ALTER TABLE `fasp`.`rm_shipment_trans` CHANGE `SHIPMENT_QTY` `SHIPMENT_QTY` BIGINT(20) UNSIGNED NULL COMMENT 'Qty of Logistics Unit in the Shipment'; 

ALTER TABLE `fasp`.`rm_shipment_trans_batch_info` CHANGE `BATCH_SHIPMENT_QTY` `BATCH_SHIPMENT_QTY` BIGINT(20) UNSIGNED NOT NULL; 
...
ALTER TABLE `fasp`.`rm_erp_order` CHANGE `QTY` `QTY` BIGINT(20) UNSIGNED NOT NULL; 

ALTER TABLE `fasp`.`rm_erp_shipment` CHANGE `DELIVERED_QTY` `DELIVERED_QTY` BIGINT(20) NULL ; 

ALTER TABLE `fasp`.`rm_shipment` CHANGE `SUGGESTED_QTY` `SUGGESTED_QTY` BIGINT(20) UNSIGNED NULL ; 

ALTER TABLE `fasp`.`rm_shipment_trans` CHANGE `PRODUCT_COST` `PRODUCT_COST` DECIMAL(24,4) UNSIGNED NULL COMMENT 'Final price of the Shipment for the Goods', CHANGE `FREIGHT_COST` `FREIGHT_COST` DECIMAL(24,4) UNSIGNED NULL COMMENT 'Cost of Freight for the Shipment'; 

ALTER TABLE `fasp`.`rm_supply_plan_amc` CHANGE `AMC` `AMC` DECIMAL(24,4) NULL , CHANGE `MOS` `MOS` DECIMAL(24,4) NULL , CHANGE `MOS_WPS` `MOS_WPS` DECIMAL(24,4) NULL , CHANGE `MIN_STOCK_QTY` `MIN_STOCK_QTY` DECIMAL(24,4) NULL , CHANGE `MIN_STOCK_MOS` `MIN_STOCK_MOS` DECIMAL(24,4) NULL , CHANGE `MAX_STOCK_QTY` `MAX_STOCK_QTY` DECIMAL(24,4) NULL , CHANGE `MAX_STOCK_MOS` `MAX_STOCK_MOS` DECIMAL(24,4) NULL ; 

ALTER TABLE `fasp`.`rm_budget` CHANGE `BUDGET_AMT` `BUDGET_AMT` DECIMAL(22,2) UNSIGNED NOT NULL COMMENT 'The total Budget amt approved for this Budget'; 


-- Pipeline script shared by palash
...
ALTER TABLE `fasp`.`qat_temp_program_planning_unit` ADD COLUMN `LOCAL_PROCUREMENT_LEAD_TIME` DECIMAL(12,2) NULL AFTER `PIPELINE_PRODUCT_ID`;

ALTER TABLE `fasp`.`qat_temp_program_planning_unit` ADD COLUMN `SHELF_LIFE` INT(10) NULL AFTER `LOCAL_PROCUREMENT_LEAD_TIME`, ADD COLUMN `CATALOG_PRICE` DECIMAL(12,2) NULL AFTER `SHELF_LIFE`;

ALTER TABLE `fasp`.`qat_temp_inventory` ADD COLUMN `MULTIPLIER` DECIMAL(12,2) NULL AFTER `PIPELINE_ID`;

ALTER TABLE `fasp`.`qat_temp_inventory` CHANGE COLUMN `REALM_COUNTRY_PLANNING_UNIT_ID` `PLANNING_UNIT_ID` VARCHAR(225) CHARACTER SET 'utf8' NULL DEFAULT NULL ;

ALTER TABLE `fasp`.`qat_temp_inventory` ADD COLUMN `REALM_COUNTRY_PLANNING_UNIT_ID` INT(11) NULL AFTER `MULTIPLIER`;


ALTER TABLE `fasp`.`adb_pipeline` ADD COLUMN `STATUS` TINYINT(1) NULL AFTER `FILE_NAME`;

ALTER TABLE `fasp`.`qat_temp_program_planning_unit` ADD COLUMN `MONTHS_IN_FUTURE_FOR_AMC` INT(11) NULL AFTER `CATALOG_PRICE`, ADD COLUMN `MONTHS_IN_PAST_FOR_AMC` INT(11) NULL AFTER `MONTHS_IN_FUTURE_FOR_AMC`; 


ALTER TABLE `fasp`.`qat_temp_program` DROP COLUMN `PLANNED_TO_DRAFT_LEAD_TIME`, CHANGE `DRAFT_TO_SUBMITTED_LEAD_TIME` `PLANNED_TO_SUBMITTED_LEAD_TIME` DECIMAL(12,2) NULL ; 
ALTER TABLE `fasp`.`qat_temp_program_planning_unit` ADD COLUMN `MULTIPLIER` INT(11) NULL AFTER `PLANNING_UNIT_ID`; 
ALTER TABLE `fasp`.`qat_temp_program_region` CHANGE `PROGRAM_ID` `PIPELINE_ID` INT(11) NULL ; 

Drop table if exists `qat_temp_data_source`;
CREATE TABLE `fasp`.`qat_temp_data_source`( `QAT_DATASOURCE_ID` INT(11) , `PIPELINE_DATA_SOURCE_TYPE` VARCHAR(500) , `PIPELINE_DATA_SOURCE_ID` VARCHAR(500) , `DATA_SOURCE_ID` INT(11) ); 
ALTER TABLE `fasp`.`qat_temp_data_source` CHANGE `QAT_DATASOURCE_ID` `QAT_DATASOURCE_ID` INT(11) NOT NULL AUTO_INCREMENT, ADD PRIMARY KEY(`QAT_DATASOURCE_ID`); 
ALTER TABLE `fasp`.`qat_temp_data_source` ADD COLUMN `PIPELINE_ID` INT(11) NULL AFTER `DATA_SOURCE_ID`; 

ALTER TABLE `fasp`.`qat_temp_program_planning_unit` CHANGE `MULTIPLIER` `MULTIPLIER` DOUBLE(12,2) NULL ; 
 ALTER TABLE `fasp`.`qat_temp_shipment` ADD COLUMN `PLANNED_DATE` DATE NULL AFTER `FREIGHT_COST`, ADD COLUMN `SUBMITTED_DATE` DATE NULL AFTER `PLANNED_DATE`, ADD COLUMN `APPROVED_DATE` DATE NULL AFTER `SUBMITTED_DATE`, ADD COLUMN `ARRIVED_DATE` DATE NULL AFTER `APPROVED_DATE`;
ALTER TABLE `fasp`.`qat_temp_shipment` CHANGE `SHIPPING_MODE` `SHIPPING_MODE` VARCHAR(10) CHARACTER SET utf8 COLLATE utf8_bin NULL ; 
ALTER TABLE `fasp`.`qat_temp_program` ADD COLUMN `SHELF_LIFE` DECIMAL(12.2) NULL AFTER `ARRIVED_TO_DELIVERED_LEAD_TIME`;  
...
INSERT INTO `fasp`.`adb_shipmentstatus`(`PipelineShipmentStatusCode`,`QAT_SHIPMENT_STATUS_ID`) VALUES ( 'A','6'); 
INSERT INTO `fasp`.`adb_shipmentstatus`(`PipelineShipmentStatusCode`,`QAT_SHIPMENT_STATUS_ID`) VALUES ( 'P','1'); 
INSERT INTO `fasp`.`adb_shipmentstatus`(`PipelineShipmentStatusCode`,`QAT_SHIPMENT_STATUS_ID`) VALUES ( 'R','7'); 
INSERT INTO `fasp`.`adb_shipmentstatus`(`PipelineShipmentStatusCode`,`QAT_SHIPMENT_STATUS_ID`) VALUES ( 'S','5'); 
INSERT INTO `fasp`.`adb_shipmentstatus`(`PipelineShipmentStatusCode`,`QAT_SHIPMENT_STATUS_ID`) VALUES ( 'O','3'); 

ALTER TABLE `fasp`.`qat_temp_consumption` ADD COLUMN `MULTIPLIER` DECIMAL(12,2) NULL AFTER `ACTUAL_FLAG`, ADD COLUMN `REALM_COUNTRY_PLANNING_UNIT_ID` INT(11) NULL AFTER `MULTIPLIER`; 
ALTER TABLE `fasp`.`qat_temp_program_planning_unit` ADD COLUMN `ACTIVE` TINYINT(1) NULL AFTER `MONTHS_IN_PAST_FOR_AMC`; 

ALTER TABLE `fasp`.`adb_datasource` ADD COLUMN `QATDataSourceLabel` VARCHAR(50) NULL AFTER `PIPELINE_ID`;
...
ALTER TABLE `fasp`.`qat_temp_program` ADD COLUMN `PROGRAM_CODE` VARCHAR(45) NULL AFTER `SHELF_LIFE`;
