...
DELIMITER $$
USE `fasp`$$
CREATE DEFINER=`faspUser`@`%` PROCEDURE `getForecastErrorUpdated`(VAR_PROGRAM_ID INT(10), VAR_VERSION_ID INT (10), VAR_VIEW_BY INT(10), VAR_UNIT_ID INT(10), VAR_START_DATE DATE, VAR_STOP_DATE DATE, VAR_REGION_IDS TEXT, VAR_EQUIVALENCY_UNIT_ID INT(10), VAR_PREVIOUS_MONTHS INT(10), VAR_DAYS_OF_STOCK_OUT TINYINT (1))
BEGIN
	...
	mn.MONTH, 
        ...
        SUM(IF(c1.CONSUMPTION_DATE=mn.MONTH, IF(@equivalencyUnitId=0, IF(@viewBy=1,	c1.ACTUAL_CONSUMPTION, c1.ACTUAL_CONSUMPTION*c1.MULTIPLIER), c1.ACTUAL_CONSUMPTION*c1.MULTIPLIER*COALESCE(eum1.CONVERT_TO_EU,eum2.CONVERT_TO_EU)),null)) `ACTUAL_CONSUMPTION`, 
        SUM(IF(c1.CONSUMPTION_DATE=mn.MONTH, c1.DAYS_OF_STOCK_OUT, null)) `DAYS_OF_STOCK_OUT`,
        SUM(IF(c1.CONSUMPTION_DATE BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths MONTH) AND mn.MONTH, ABS(c1.FORECAST_CONSUMPTION-IF(@daysOfStockOut,c1.ADJUSTED_ACTUAL_CONSUMPTION,c1.ACTUAL_CONSUMPTION)), null)) `TOTAL_FORECAST_MINUS_ACTUAL`,
        SUM(IF(c1.CONSUMPTION_DATE	 BETWEEN SUBDATE(mn.MONTH, INTERVAL @previousMonths MONTH) AND mn.MONTH, IF(@daysOfStockOut,c1.ADJUSTED_ACTUAL_CONSUMPTION,c1.ACTUAL_CONSUMPTION), null)) `TOTAL_ACTUAL`
    FROM mn 
