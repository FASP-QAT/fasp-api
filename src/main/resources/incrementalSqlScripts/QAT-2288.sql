        ...
        FROM (
            ...
            FROM (
                SELECT 
                    ...
                    SUM(IF(ct.ACTUAL_FLAG=1, COALESCE(ctbi.CONSUMPTION_QTY * rcpu.MULTIPLIER, ct.CONSUMPTION_QTY), null)) `ACTUAL_CONSUMPTION`, 
                    SUM(IF(ct.ACTUAL_FLAG=0, COALESCE(ctbi.CONSUMPTION_QTY * rcpu.MULTIPLIER, ct.CONSUMPTION_QTY), null)) `FORECASTED_CONSUMPTION`
                FROM (
            ...
            SELECT 
                ...
                SUM(IF(st.SHIPMENT_STATUS_ID IN (1,3,4,5,6,7,9), COALESCE(stbi.BATCH_SHIPMENT_QTY * rcpu.MULTIPLIER ,st.SHIPMENT_QTY),0)) `SHIPMENT`, 
                SUM(IF(st.SHIPMENT_STATUS_ID IN (3,4,5,6,7,9), COALESCE(stbi.BATCH_SHIPMENT_QTY * rcpu.MULTIPLIER ,st.SHIPMENT_QTY), 0)) `SHIPMENT_WPS`, 
