...
DROP USER IF EXISTS 'faspUser'@'%';
CREATE USER 'faspUser'@'%' IDENTIFIED BY 'faspP@ssw0rd';
GRANT ALL PRIVILEGES ON *.* TO 'faspUser'@'%';
GRANT CREATE VIEW, CREATE ROUTINE ON *.* TO 'faspUser'@'%';
FLUSH PRIVILEGES;
...
select 
    ...
    group_concat(`pha`.`HEALTH_AREA_ID` separator ',') AS `HEALTH_AREA_ID`,
...
select 
    ...
    `c`.`COUNTRY_CODE2` AS `COUNTRY_CODE2`,
    `c`.`CURRENCY_ID` AS `CURRENCY_ID`,
    `c`.`ACTIVE` AS `ACTIVE`,
    `c`.`CREATED_BY` AS `CREATED_BY`,
    `c`.`CREATED_DATE` AS `CREATED_DATE`,
    `c`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `c`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `cl`.`LABEL_EN` AS `LABEL_EN`,
    `cl`.`LABEL_FR` AS `LABEL_FR`,
    `cl`.`LABEL_SP` AS `LABEL_SP`,
    `cl`.`LABEL_PR` AS `LABEL_PR`
from (`ap_country` `c` 
...
select 
    ...
    `c`.`LABEL_ID` AS `LABEL_ID`,
    `c`.`CONVERSION_RATE_TO_USD` AS `CONVERSION_RATE_TO_USD`,
...
from (`ap_currency` `c` 
left join `ap_label` `l` on((`c`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_data_source` AS 
select 
    `ds`.`DATA_SOURCE_ID` AS `DATA_SOURCE_ID`,
    `ds`.`REALM_ID` AS `REALM_ID`,
    `ds`.`PROGRAM_ID` AS `PROGRAM_ID`,
    `ds`.`DATA_SOURCE_TYPE_ID` AS `DATA_SOURCE_TYPE_ID`,
    `ds`.`LABEL_ID` AS `LABEL_ID`,
    `ds`.`ACTIVE` AS `ACTIVE`,
    `ds`.`CREATED_BY` AS `CREATED_BY`,
    `ds`.`CREATED_DATE` AS `CREATED_DATE`,
    `ds`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `ds`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `dsl`.`LABEL_EN` AS `LABEL_EN`,
    `dsl`.`LABEL_FR` AS `LABEL_FR`,
    `dsl`.`LABEL_SP` AS `LABEL_SP`,
    `dsl`.`LABEL_PR` AS `LABEL_PR`
...
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_data_source_type` AS 
select 
    `dst`.`DATA_SOURCE_TYPE_ID` AS `DATA_SOURCE_TYPE_ID`,
    `dst`.`REALM_ID` AS `REALM_ID`,
    `dst`.`LABEL_ID` AS `LABEL_ID`,
    `dst`.`ACTIVE` AS `ACTIVE`,
    `dst`.`CREATED_BY` AS `CREATED_BY`,
    `dst`.`CREATED_DATE` AS `CREATED_DATE`,
    `dst`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `dst`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `l`.`LABEL_EN` AS `LABEL_EN`,
    `l`.`LABEL_FR` AS `LABEL_FR`,
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_data_source_type` `dst` 
left join `ap_label` `l` on((`dst`.`LABEL_ID` = `l`.`LABEL_ID`)));
...
select 
    ...
    group_concat(`euha`.`HEALTH_AREA_ID` separator ',') AS `HEALTH_AREA_ID`,
...
from (`ap_extrapolation_method` `em` 
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`ap_forecast_method_type` `ut` 
left join `ap_label` `l` on((`ut`.`LABEL_ID` = `l`.`LABEL_ID`)));
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_forecast_tree_node` `tn` 
left join `ap_label` `l` on((`tn`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_forecasting_unit` AS 
select 
    `fu`.`FORECASTING_UNIT_ID` AS `FORECASTING_UNIT_ID`,
    `fu`.`REALM_ID` AS `REALM_ID`,
    `fu`.`PRODUCT_CATEGORY_ID` AS `PRODUCT_CATEGORY_ID`,
    `fu`.`TRACER_CATEGORY_ID` AS `TRACER_CATEGORY_ID`,
    `fu`.`GENERIC_LABEL_ID` AS `GENERIC_LABEL_ID`,
    `fu`.`LABEL_ID` AS `LABEL_ID`,
    `fu`.`UNIT_ID` AS `UNIT_ID`,
    `fu`.`ACTIVE` AS `ACTIVE`,
    `fu`.`CREATED_BY` AS `CREATED_BY`,
    `fu`.`CREATED_DATE` AS `CREATED_DATE`,
    `fu`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `fu`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `ful`.`LABEL_EN` AS `LABEL_EN`,
    `ful`.`LABEL_FR` AS `LABEL_FR`,
    `ful`.`LABEL_SP` AS `LABEL_SP`,
    `ful`.`LABEL_PR` AS `LABEL_PR`
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_funding_source_type` `pat` 
left join `ap_label` `l` on((`pat`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_health_area` AS 
select 
    `ha`.`HEALTH_AREA_ID` AS `HEALTH_AREA_ID`,
    `ha`.`REALM_ID` AS `REALM_ID`,
    `ha`.`LABEL_ID` AS `LABEL_ID`,
    `ha`.`HEALTH_AREA_CODE` AS `HEALTH_AREA_CODE`,
    `ha`.`ACTIVE` AS `ACTIVE`,
    `ha`.`CREATED_BY` AS `CREATED_BY`,
    `ha`.`CREATED_DATE` AS `CREATED_DATE`,
    `ha`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `ha`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `hal`.`LABEL_EN` AS `LABEL_EN`,
    `hal`.`LABEL_FR` AS `LABEL_FR`,
    `hal`.`LABEL_SP` AS `LABEL_SP`,
    `hal`.`LABEL_PR` AS `LABEL_PR`
...
select 
    ...
    `ot`.`LABEL_EN` AS `TYPE_LABEL_EN`,
    `ot`.`LABEL_FR` AS `TYPE_LABEL_FR`,
    `ot`.`LABEL_SP` AS `TYPE_LABEL_SP`,
    `ot`.`LABEL_PR` AS `TYPE_LABEL_PR`,
    `o`.`ACTIVE` AS `ACTIVE`,
    `o`.`CREATED_BY` AS `CREATED_BY`,
    `o`.`CREATED_DATE` AS `CREATED_DATE`,
    `o`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
...
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_planning_unit` AS 
select 
    `pu`.`PLANNING_UNIT_ID` AS `PLANNING_UNIT_ID`,
    `pu`.`FORECASTING_UNIT_ID` AS `FORECASTING_UNIT_ID`,
    `pu`.`LABEL_ID` AS `LABEL_ID`,
    `pu`.`UNIT_ID` AS `UNIT_ID`,
    `pu`.`MULTIPLIER` AS `MULTIPLIER`,
    `pu`.`ACTIVE` AS `ACTIVE`,
    `pu`.`CREATED_BY` AS `CREATED_BY`,
    `pu`.`CREATED_DATE` AS `CREATED_DATE`,
    `pu`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `pu`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `pul`.`LABEL_EN` AS `LABEL_EN`,
    `pul`.`LABEL_FR` AS `LABEL_FR`,
    `pul`.`LABEL_SP` AS `LABEL_SP`,
    `pul`.`LABEL_PR` AS `LABEL_PR`
...
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_problem_category` AS 
select 
    `pc`.`PROBLEM_CATEGORY_ID` AS `PROBLEM_CATEGORY_ID`,
    `l`.`LABEL_ID` AS `LABEL_ID`,
    `l`.`LABEL_EN` AS `LABEL_EN`,
    `l`.`LABEL_FR` AS `LABEL_FR`,
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`ap_problem_category` `pc` 
left join `ap_label` `l` on((`pc`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_problem_criticality` AS 
select 
    `cr`.`CRITICALITY_ID` AS `CRITICALITY_ID`,
    `cr`.`LABEL_ID` AS `LABEL_ID`,
    `cr`.`COLOR_HTML_CODE` AS `COLOR_HTML_CODE`,
    `crl`.`LABEL_EN` AS `LABEL_EN`,
    `crl`.`LABEL_FR` AS `LABEL_FR`,
    `crl`.`LABEL_SP` AS `LABEL_SP`,
    `crl`.`LABEL_PR` AS `LABEL_PR`
from (`ap_problem_criticality` `cr` 
left join `ap_label` `crl` on((`cr`.`LABEL_ID` = `crl`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_problem_status` AS 
select 
    `ps`.`PROBLEM_STATUS_ID` AS `PROBLEM_STATUS_ID`,
    `ps`.`USER_MANAGED` AS `USER_MANAGED`,
    `ps`.`LABEL_ID` AS `LABEL_ID`,
    `psl`.`LABEL_EN` AS `LABEL_EN`,
    `psl`.`LABEL_FR` AS `LABEL_FR`,
    `psl`.`LABEL_SP` AS `LABEL_SP`,
    `psl`.`LABEL_PR` AS `LABEL_PR`
from (`ap_problem_status` `ps` 
left join `ap_label` `psl` on((`ps`.`LABEL_ID` = `psl`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_problem_type` AS 
select 
    `pt`.`PROBLEM_TYPE_ID` AS `PROBLEM_TYPE_ID`,
    `pt`.`LABEL_ID` AS `LABEL_ID`,
    `ptl`.`LABEL_EN` AS `LABEL_EN`,
    `ptl`.`LABEL_FR` AS `LABEL_FR`,
    `ptl`.`LABEL_SP` AS `LABEL_SP`,
    `ptl`.`LABEL_PR` AS `LABEL_PR`
from (`ap_problem_type` `pt` 
left join `ap_label` `ptl` on((`pt`.`LABEL_ID` = `ptl`.`LABEL_ID`)));
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_procurement_agent_type` `pat` 
left join `ap_label` `l` on((`pat`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_procurement_unit` AS 
select 
    `pu`.`PROCUREMENT_UNIT_ID` AS `PROCUREMENT_UNIT_ID`,
    `pu`.`PLANNING_UNIT_ID` AS `PLANNING_UNIT_ID`,
    `pu`.`LABEL_ID` AS `LABEL_ID`,
    `pu`.`UNIT_ID` AS `UNIT_ID`,
    `pu`.`MULTIPLIER` AS `MULTIPLIER`,
    `pu`.`SUPPLIER_ID` AS `SUPPLIER_ID`,
    `pu`.`WIDTH_QTY` AS `WIDTH_QTY`,
    `pu`.`HEIGHT_QTY` AS `HEIGHT_QTY`,
    `pu`.`LENGTH_UNIT_ID` AS `LENGTH_UNIT_ID`,
    `pu`.`LENGTH_QTY` AS `LENGTH_QTY`,
    `pu`.`WEIGHT_QTY` AS `WEIGHT_QTY`,
    `pu`.`WEIGHT_UNIT_ID` AS `WEIGHT_UNIT_ID`,
    `pu`.`VOLUME_QTY` AS `VOLUME_QTY`,
    `pu`.`VOLUME_UNIT_ID` AS `VOLUME_UNIT_ID`,
    `pu`.`UNITS_PER_CASE` AS `UNITS_PER_CASE`,
    `pu`.`UNITS_PER_PALLET_EURO1` AS `UNITS_PER_PALLET_EURO1`,
    `pu`.`UNITS_PER_PALLET_EURO2` AS `UNITS_PER_PALLET_EURO2`,
    `pu`.`UNITS_PER_CONTAINER` AS `UNITS_PER_CONTAINER`,
    `pu`.`LABELING` AS `LABELING`,
    `pu`.`ACTIVE` AS `ACTIVE`,
    `pu`.`CREATED_BY` AS `CREATED_BY`,
    `pu`.`CREATED_DATE` AS `CREATED_DATE`,
    `pu`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    ...
    `pul`.`LABEL_SP` AS `LABEL_SP`,
    `pul`.`LABEL_PR` AS `LABEL_PR`
from (`rm_procurement_unit` `pu` 
left join `ap_label` `pul` on((`pu`.`LABEL_ID` = `pul`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_product_category` AS 
select 
    `pc`.`PRODUCT_CATEGORY_ID` AS `PRODUCT_CATEGORY_ID`,
    `pc`.`REALM_ID` AS `REALM_ID`,
    `pc`.`LABEL_ID` AS `LABEL_ID`,
    `pc`.`PARENT_PRODUCT_CATEGORY_ID` AS `PARENT_PRODUCT_CATEGORY_ID`,
    `pc`.`SORT_ORDER` AS `SORT_ORDER`,
    `pc`.`ACTIVE` AS `ACTIVE`,
    `pc`.`CREATED_BY` AS `CREATED_BY`,
    `pc`.`CREATED_DATE` AS `CREATED_DATE`,
    `pc`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `pc`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `pcl`.`LABEL_EN` AS `LABEL_EN`,
    `pcl`.`LABEL_FR` AS `LABEL_FR`,
    `pcl`.`LABEL_SP` AS `LABEL_SP`,
    `pcl`.`LABEL_PR` AS `LABEL_PR`
...
select 
    ...
    `r`.`REALM_COUNTRY_ID` AS `REALM_COUNTRY_ID`,
    `r`.`LABEL_ID` AS `LABEL_ID`,
    `r`.`CAPACITY_CBM` AS `CAPACITY_CBM`,
    `r`.`GLN` AS `GLN`,
    `r`.`ACTIVE` AS `ACTIVE`,
    `r`.`CREATED_BY` AS `CREATED_BY`,
    `r`.`CREATED_DATE` AS `CREATED_DATE`,
...
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_shipment_status` AS 
select 
    `ss`.`SHIPMENT_STATUS_ID` AS `SHIPMENT_STATUS_ID`,
    `ss`.`LABEL_ID` AS `LABEL_ID`,
    `ss`.`ACTIVE` AS `ACTIVE`,
    `ss`.`CREATED_BY` AS `CREATED_BY`,
    `ss`.`CREATED_DATE` AS `CREATED_DATE`,
    `ss`.`LAST_MODIFIED_BY` AS `LAST_MODIFIED_BY`,
    `ss`.`LAST_MODIFIED_DATE` AS `LAST_MODIFIED_DATE`,
    `sl`.`LABEL_EN` AS `LABEL_EN`,
    `sl`.`LABEL_FR` AS `LABEL_FR`,
    `sl`.`LABEL_SP` AS `LABEL_SP`,
    `sl`.`LABEL_PR` AS `LABEL_PR`
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_forecast_tree_level` `tl` 
...
select 
    ...
    `l`.`LABEL_PR` AS `LABEL_PR`,
    `tt`.`NOTES` AS `NOTES`
from (`rm_tree_template` `tt` 
left join `ap_label` `l` on((`tt`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_tree_template_level` AS 
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_tree_template_level` `tl` 
left join `ap_label` `l` on((`tl`.`LABEL_ID` = `l`.`LABEL_ID`)));
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`rm_tree_template_node` `ttn` 
...
select 
    ...
    `u`.`DIMENSION_ID` AS `DIMENSION_ID`,
    `u`.`LABEL_ID` AS `LABEL_ID`,
    ...
    `ul`.`LABEL_EN` AS `LABEL_EN`,
    `ul`.`LABEL_FR` AS `LABEL_FR`,
    `ul`.`LABEL_SP` AS `LABEL_SP`,
    `ul`.`LABEL_PR` AS `LABEL_PR`
from (`ap_unit` `u` 
...
select 
    ...
    `l`.`LABEL_SP` AS `LABEL_SP`,
    `l`.`LABEL_PR` AS `LABEL_PR`
from (`ap_usage_type` `ut` 
left join `ap_label` `l` on((`ut`.`LABEL_ID` = `l`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_version_status` AS 
select 
    `vs`.`VERSION_STATUS_ID` AS `VERSION_STATUS_ID`,
    `vs`.`LABEL_ID` AS `LABEL_ID`,
    `vsl`.`LABEL_EN` AS `LABEL_EN`,
    `vsl`.`LABEL_FR` AS `LABEL_FR`,
    `vsl`.`LABEL_SP` AS `LABEL_SP`,
    `vsl`.`LABEL_PR` AS `LABEL_PR`
from (`ap_version_status` `vs` 
left join `ap_label` `vsl` on((`vs`.`LABEL_ID` = `vsl`.`LABEL_ID`)));

CREATE OR REPLACE 
DEFINER = 'faspUser'@'%'
SQL SECURITY DEFINER
VIEW `vw_version_type` AS 
select 
    `vt`.`VERSION_TYPE_ID` AS `VERSION_TYPE_ID`,
    `vt`.`LABEL_ID` AS `LABEL_ID`,
    `vtl`.`LABEL_EN` AS `LABEL_EN`,
    `vtl`.`LABEL_FR` AS `LABEL_FR`,
    `vtl`.`LABEL_SP` AS `LABEL_SP`,
    `vtl`.`LABEL_PR` AS `LABEL_PR`
from (`ap_version_type` `vt` 
...
VALUES
    (1, 1, 'Alexio Danje', '$2a$10$4PlC61faEmVXr28xkAi3w.zUBq3cJ6OOWtcJTLZucWm6.Xby8whyG', 'alexiodanje@gmail.com', '96371396638', 1, 1, 0, '2035-07-08 11:37:02', '2035-07-08 11:37:27', '2024-07-08 11:37:27', 1, '2020-02-12 12:00:00', 1, '2024-07-08 11:38:38', 1, 'qm:98503734-59b7-46ea-bb68-137900ca2d4b:702316ba-5743-4c6b-8eb3-56334b63cc34', 2, 1, 0);
...
VALUES 
    (1, 'en', 29207, 'us', 1, 1, '2020-02-20 12:00:00', 1, '2020-02-20 12:00:00'),
    (2, 'fr', 29208, 'wf', 1, 1, '2020-02-20 12:00:00', 1, '2020-02-20 12:00:00'),
    (3, 'sp', 29209, 'es', 1, 1, '2020-02-20 12:00:00', 1, '2020-02-20 12:00:00'),
    (4, 'pr', 29210, 'pt', 1, 1, '2020-02-20 12:00:00', 1, '2020-02-20 12:00:00');
...
VALUES
    ...
    ('ROLE_COUNTRY_INTEGRATION',337692,9,'2024-07-30 07:10:09',9,'2024-07-30 07:10:09'),
    ('ROLE_DATASET_ADMIN',36726,1,'2022-05-15 10:57:35',1,'2022-05-15 10:57:35'),
    ('ROLE_DATASET_USER',36727,1,'2022-05-15 10:57:35',1,'2022-05-15 10:57:35'),
    ('ROLE_FORECAST_PROGRAM_ADMIN_(NO_UPLOAD)',309645,36,'2024-01-25 13:10:38',36,'2024-01-25 13:10:38'),
    ('ROLE_FORECAST_VIEWER',37297,1,'2022-05-24 08:16:08',1,'2022-05-24 08:16:08'),
    ('ROLE_GUEST_USER',30,1,'2020-02-20 12:00:00',1,'2020-02-20 12:00:00'),
    ('ROLE_INTERNAL_USER',31078,1,'2021-06-15 07:25:22',1,'2021-06-15 07:25:22'),
    ('ROLE_PROGRAM_ADMIN',27,1,'2020-02-20 12:00:00',1,'2020-02-20 12:00:00'),
    ('ROLE_PROGRAM_USER',29,1,'2020-02-20 12:00:00',1,'2020-02-20 12:00:00'),
    ('ROLE_REALM_ADMIN',26,1,'2020-02-20 12:00:00',1,'2020-02-20 12:00:00'),
    ('ROLE_REPORT_USER',31,1,'2020-02-20 12:00:00',1,'2020-02-20 12:00:00'),
    ('ROLE_SP_PROGRAM_LEVEL_ADMIN_(NO_UPLOAD)',309644,36,'2024-01-25 13:06:27',36,'2024-01-25 13:06:27'),
    ('ROLE_SUPPLY_PLAN_REVIEWER',28,1,'2020-02-20 12:00:00',1,'2020-02-20 12:00:00'),
    ('ROLE_TRAINER_ADMIN',78032,36,'2023-01-26 15:31:47',36,'2023-01-26 15:31:47'),
    ('ROLE_VIEW_DATA_ENTRY',28543,3,'2020-12-23 14:25:06',3,'2020-12-23 14:25:06');
...
INSERT INTO `ap_label_source`
(`SOURCE_ID`, `SOURCE_DESC`, `SOURCE_TEXT`) 
VALUES 
    (1,'ap_country','Country'),
    (2,'rm_realm','Realm'),
    (3,'ap_currency','Currency'),
    (4,'ap_dimension','Dimension'),
    (5,'ap_version_type','Program version type'),
    (6,'ap_version_status','Program version status'),
    (7,'ap_shipment_status','Shipment status'),
    (8,'us_role','Role'),
    (9,'rm_health_area','Health Area'),
    (10,'rm_organisation','Organisation'),
    (11,'rm_region','Region'),
    (12,'rm_procurement_agent','Procurement Agent'),
    (13,'rm_funding_source','Funding Source'),
    (14,'rm_budget','Budget'),
    (15,'rm_data_source','Data Source'),
    (16,'rm_data_source_type','Data Source type'),
    (17,'ap_unit','Unit'),
    (18,'rm_program','Program'),
    (19,'ap_problem_type','Problem type'),
    (20,'ap_problem_status','Problem status'),
    (21,'ap_problem_criticallity','Problem criticality'),
    (22,'ap_problem','Problem'),
    (23,'ap_problem action','Problem action'),
    (24,'us_business_function','Business function'),
    (25,'unknown',''),
    (26,'rm_product_category','Product category'),
    (27,'rm_tracer_category','Tracer category'),
    (28,'rm_forecasting_unit','Forecasting Unit'),
    (29,'rm_forecasting_unit genericName','Forecasting Unit - Generic name'),
    (30,'rm_planning_unit','Planning Unit'),
    (31,'rm_procurement_unit','Procurement Unit'),
    (32,'rm_realm_country_planning_unit','Alternate Reporting Unit'),
    (33,'rm_supplier','Supplier'),
    (34,'ap_problem_category','Problem category'),
    (35,'ap_language','Language'),
    (36,'ap_notification_type','Notification type'),
    (37,'rm_organisation_type','Organisation type'),
    (38,'ap_usage_type','Usage type'),
    (39,'ap_node_type','Node type'),
    (40,'ap_usage_period','Usage period'),
    (41,'ap_modeling_type','Modeling type'),
    (42,'ap_forecast_method_type','Forecast method type'),
    (43,'rm_forecast_method','Forecast method'),
    (44,'rm_equivalency_unit','Equivalency Unit'),
    (45,'rm_tree_template','Tree template'),
    (46,'rm_tree_template_node','Tree template - Node'),
    (47,'rm_usage_template','Usage template'),
    (48,'rm_forecast_tree','Forecast tree'),
    (49,'rm_forecast_tree_node','Forecast tree - Node'),
    (50,'rm_scenario','Forecast tree - Scenario'),
    (51,'rm_dataset_planning_unit','Dataset Planning Unit'),
    (52,'ap_extrapolation_method','Extrapolation method'),
    (53,'rm_forecast_tree_level','Forecast tree - Level'),
    (54,'rm_tree_template_level','Tree template - Level'),
    (55,'rm_procurement_agent_type','Procurement Agent Type'),
    (56,'rm_funding_source_type','Funding Source Type');
